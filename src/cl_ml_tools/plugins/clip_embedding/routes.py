...
def create_router(
    ...
    Returns:
        Configured APIRouter with MobileCLIP embedding endpoint
    """
    router = APIRouter()

    @router.post("/jobs/clip_embedding", response_model=JobCreatedResponse)
    async def create_clip_embedding_job(
        file: Annotated[UploadFile, File(description="Image file for semantic similarity embedding")],
        normalize: Annotated[
        ...
        """Create a new MobileCLIP embedding job.
        ...
        job = Job(
            ...
            params={
                ...
                "output_paths": [],  # MobileCLIP embedding doesn't produce output files
                "normalize": normalize,
            },
        )
